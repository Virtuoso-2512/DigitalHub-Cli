...
export default function PostManage(props){
    const [name, setName] = useState(""),
          ...
          {InstituteName, post, get} = GlobalState();

    useEffect(async() => {
      const postReq = await get("post/"+props.match.params.id);
      setPostData(postReq.post);
      setName(postReq.post.name);
      setPost(postReq.post.type);
      setPerm(postReq.post.permissions);
      setCount(postReq.count);
    }, [])

    const createPost = async() => {
        setLoading(true);
        const res = await post("post/save?id="+props.match.params.id, {name, permissions:perm, type});
        setLoading(false);
        
        if (!res.success) return toast("Error in saving the Post !");
    ...
    return <div style={{marginLeft:"1vw",display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center"}}>
        <h2 style={{textAlign:"center",margin:"1vw auto"}}>Manage Post</h2>
        <span>Posts edited here will be visible and could be used only to Faculty directly under the <b>{InstituteName}</b></span>
        ...
        <Button disabled={loading || !(name)} onClick={createPost} variant={1} style={{ margin: "0 10px" }}>Save</Button>
        <div style={{width:"85vw",display:"flex",justifyContent:"space-around",marginLeft:"2.5vw"}}>
            <h4>Created at : {new Date(postData.createdAt).toLocaleString()}</h4>
            <h4 style={{fontFamily:"Poppins"}}>{count} Faculty use this Post</h4>
            <h4>Last Updated at : {new Date(postData.updatedAt).toLocaleString()}</h4>
        </div>
        <div style={{textAlign:"left",width:"70vw",margin:"auto"}}>
            ...
            <Permission parent={perm} nest="crm" onClick={handlePerm} state="relManager" title="Relationship Managers" description="Allow these users to act as Relationship Managers."/>
        </div>
    </div>;
}
